...
def rnn_cell_forward(xt, a_prev, parameters):
	...
	prod_1 = np.dot(Waa, a_prev)
	prod_2 = Wax[:, xt.reshape(-1)]
	a_next = np.tanh(prod_1 + prod_2 + ba)
...
def compute_cost(y_pred, Y):
	...
	cost = 0
	for t in range(T_y):
		y_pred_t = y_pred[:, :, t][Y[:, :, t].reshape(-1), np.array(range(m))]
		cost += np.sum(Y[:, :, t] * np.log(y_pred_t))
		
	cost = (-1/m) * cost
	return cost

def rnn_cell_backward(da_next, dZt, cache, vocab_size):
    ...
    dz = 1 - np.tanh(prod_1 + prod_2 + ba) ** 2

    dxt = np.dot(Wax.T, da_next * dz)
    dWax = (1/m) * np.dot(da_next * dz, one_hot_2d(xt, vocab_size).T)
...
def rnn_backward(X, Y, y_pred, caches, vocab_size):
...
##    dZ = y_pred - Y
    ...
    for t in reversed(range(T_x)):
        ...
        dZt[Y[:, :, t].reshape(-1), np.array(range(m))] -= 1
        
        da_next = np.dot(parameters['Wya'].T, dZt) + da_next

        gradients = rnn_cell_backward(da_next, dZt, caches[t], vocab_size)
...
orig_data = pd.read_csv('/Users/shz204/Downloads/names.csv')
data = orig_data
data.shape
...
data = data[data.apply(lambda x: len(x) == 5)]
...
def optimize(X, Y, a_prev, vocab_size, parameters, learning_rate):

    a, y_pred, caches = rnn_forward(X, a_prev, parameters)

    cost = compute_cost(y_pred, Y)

    gradients = rnn_backward(X, Y, y_pred, caches, vocab_size)
...
def model(X, Y, idx_to_chr, chr_to_idx, num_iterations = 35000, n_a = 50, sample_size = 7, vocab_size = 27, learning_rate = 0.001):
    ...
    for j in range(num_iterations): 
        ...
        curr_loss, gradients, a_prev = optimize(X, Y, a_prev, vocab_size, parameters, learning_rate)
        loss = smooth(loss, curr_loss)

        # Every 2000 Iteration, generate "n" characters thanks to sample() to check if the model is learning properly
        if (j % 1000 == 0) or (j == num_iterations-1):
...
parameters = model(X_tokens, Y_tokens, idx_to_chr, chr_to_idx, num_iterations = 50000, n_a = 64, sample_size = 7, vocab_size = vocab_size, learning_rate = 0.01)
